...
export default function Admin() {
  ...
  if (!isAuth) return (
    <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0f172a' }}>
      <div style={{ background: '#1e293b', padding: 30, borderRadius: 20, width: '90%', maxWidth: 400 }}>
        ...
        <input type="password" placeholder="Enter Password" onChange={e => setPass(e.target.value)} style={inStyle} />
  ...
  return (
    <div style={{ padding: 20, maxWidth: 600, margin: '0 auto', direction: 'rtl' }}>
      ...
      {activeTab === 'files' && (
        <div style={cardStyle}>
          ...
          <button onClick={async () => {
             ...
             if (!error) { alert('تم الرفع! ✅'); setFileName(''); setFileUrl(''); }
      ...
      {activeTab === 'schedules' && (
        <div style={cardStyle}>
          <h3 style={{color: '#38bdf8'}}>إضافة جدول</h3>
      ...
      {activeTab === 'checklist' && (
        <div style={cardStyle}>
          <h3 style={{color: '#38bdf8'}}>إضافة تحديد جديد</h3>
          ...
          <input placeholder="اسم الدرس" value={taskText} onChange={e => setTaskText(e.target.value)} style={inStyle} />
          <button onClick={async () => {
...
const tabStyle = { padding: '10px 20px', borderRadius: 12, border: 'none', color: '#fff', cursor: 'pointer', whiteSpace: 'nowrap' }
