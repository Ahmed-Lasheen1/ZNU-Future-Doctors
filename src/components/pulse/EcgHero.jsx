...
// the hero image. Clips the traveling glow to the real tube's EXACT
// shape/width, regardless of how wide the animated strokes below are
// drawn — only the part overlapping the real line ever shows.
...
// from the source pixels — used to drive the glow's motion as a single
// point along the real arc length (so it can't split into two through
// the sharp central spike, unlike a horizontally-sweeping band).
...
// Drawn generously wider than the real tube, then cropped down to the
// real silhouette by the mask — so the visible width is always
    ...
    <div style={{
      ...
      <style>{`
        @keyframes pulseHeroDash {
          0%   { stroke-dashoffset: ${DASH_TOTAL}; }
          100% { stroke-dashoffset: 0; }
        }
        .pulse-hero-beam-layer {
          stroke-dasharray: ${DASH_ON} ${DASH_GAP};
