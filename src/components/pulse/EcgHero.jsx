...
// the hero image. Used to clip the glow to the exact real tube
// shape/width — this is why the light is guaranteed to be exactly as
// wide as the real pulse everywhere, background-agnostic.
...
// from the source pixels (not hand-fit bezier curves). Used here as a
// MOTION PATH — a single glowing blob travels along it via
// animateMotion, which moves at constant speed along the path's own
// arc length. Because it's always exactly one point in time at exactly
// one position, it is structurally incapable of "splitting into two"
// the way a horizontally-sweeping band did through the sharp central
// spike (where a vertical band would overlap both the up-stroke and
...
export default function EcgHero({ height = 220 }) {
  ...
  return (
    <div style={{
      ...
      >
        <defs>
          <mask id="pulseLineMask2" maskUnits="userSpaceOnUse" x="0" y="0" width={ART_WIDTH} height={ART_HEIGHT}>
            <image href={LINE_MASK} x="0" y="0" width={ART_WIDTH} height={ART_HEIGHT} />
          </mask>

          {/* Soft round glow — a radial gradient rather than a flat
              circle, so its edges fade smoothly instead of showing a
              hard disc boundary. */}
          <radialGradient id="pulseGlowCore" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#ffffff" stopOpacity="1" />
            <stop offset="45%" stopColor="#bdeeff" stopOpacity="0.95" />
            <stop offset="100%" stopColor="#3ad1ff" stopOpacity="0" />
          </radialGradient>
          <radialGradient id="pulseGlowHalo" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="#7fe6ff" stopOpacity="0.9" />
            <stop offset="60%" stopColor="#3ad1ff" stopOpacity="0.4" />
            <stop offset="100%" stopColor="#3ad1ff" stopOpacity="0" />
          </radialGradient>
          ...
          <filter id="pulseBlurTight" x="-150%" y="-150%" width="400%" height="400%">
        ...
        {!reduced && (
          <g mask="url(#pulseLineMask2)">
            {/* Wide soft halo — travels the exact real centerline via
                animateMotion (constant speed along arc length), so it
                is always a single glow at a single position: it cannot
                appear on both legs of the sharp spike at once the way
                a sweeping vertical band could. Clipped to the exact
                real tube shape by the mask, so it always reads as
                exactly the pulse's own width, never wider or thinner. */}
            <ellipse rx="46" ry="46" fill="url(#pulseGlowHalo)" filter="url(#pulseBlurSoft)">
              <animateMotion dur={BEAM_DURATION} repeatCount="indefinite" rotate="0">
                <mpath href={`#pulseMotionPath`} />
              </animateMotion>
            </ellipse>
            {/* Bright electric core, same motion, tighter and brighter */}
            <ellipse rx="24" ry="24" fill="url(#pulseGlowCore)" filter="url(#pulseBlurTight)">
              <animateMotion dur={BEAM_DURATION} repeatCount="indefinite" rotate="0">
                <mpath href={`#pulseMotionPath`} />
              </animateMotion>
            </ellipse>
        ...
        {/* Hidden path used purely as the motion guide above */}
        <path id="pulseMotionPath" d={CENTER_PATH} fill="none" stroke="none" />
      </svg>
    </div>
  )
}
